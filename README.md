...
- **3종 소스 자동 믹스**: 정책 문서(근거) + 실제 구현 코드(엣지 케이스 검증) + 화면 이미지(시각 앵커)
...
- [ ] 프로젝트 폴더의 `assets/` 또는 `mockup/` 아래에 **화면 이미지**(PNG)가 5장 이상 있다
...
| 덱 | 소스 구성 | 결과 |
|----|---------|------|
| 정책·개념 중심 가이드 | 정책 md 1 + 코드 md 6 | PDF 14MB, 17장 |
| 화면별 상세 가이드 | 정책 md 1 + 화면 PNG 8 | PDF 17MB, 22장 |
| 이 스킬 자체 소개 자료 | 튜토리얼 md 1 | PDF 23MB, 22장 |
