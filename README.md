...
- **두 가지 생성 모드**: 이미지 없으면 **정책·코드 모드**(텍스트 중심), 이미지 있으면 **화면별 상세 모드**(실제 스크린샷 포함)
...
> ⚠️ **중요**: NotebookLM은 이미지를 **자동 생성하지 않는다**. 실제 UI 스크린샷이 슬라이드에 들어가려면 PNG를 직접 공급해야 한다. 코드만 주면 NotebookLM이 코드를 읽고 **텍스트·도식으로 화면 구조를 해석**할 뿐이다.
...
**선택 — 어떤 모드로 만들지 결정**
- [ ] **모드 A (정책·코드)**: 이미지 준비 불필요. 텍스트·도식 중심 슬라이드로 충분할 때
- [ ] **모드 B (화면별 상세)**: `assets/` 또는 `mockup/` 아래 **PNG 5장 이상** 준비. 실제 UI 스크린샷이 필요할 때

> 💡 모드는 이미지 유무로 자동 결정됩니다. 요청 시 "화면 스크린샷 포함해줘"처럼 명시하면 더 확실합니다.
...
| 덱 | 모드 | 소스 구성 | 결과 |
|----|------|---------|------|
| 운영팀 가이드 (정책·개념 중심) | **A (이미지 없음)** | 정책 md 1 + 코드 md 6 | PDF 14MB, 17장 — 텍스트·도식 중심 |
| 화면별 상세 가이드 | **B (이미지 있음)** | 정책 md 1 + 화면 PNG 8 | PDF 17MB, 22장 — 실제 스크린샷 포함 |
| 이 스킬 자체 소개 자료 | A | 튜토리얼 md 1 | PDF 23MB, 22장 |

> 💡 첫 번째 덱(운영팀 가이드)은 **이미지를 넣지 않고도** 만들어졌습니다. NotebookLM이 코드·정책을 읽고 화면 구조를 텍스트로 해석해준 것이지, 실제 UI 스크린샷이 포함된 건 아닙니다. 고객사 대면용으로 실제 화면이 필요하면 두 번째 덱처럼 PNG를 공급하세요.
